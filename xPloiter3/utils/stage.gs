...
promptMsg = "Up and Down to Scroll, Enter to Select, Backspace to Go Back".csize(50,true) + char(10)
...
Stage.network = function(networks)
    ...
    choices.push("New")
    choices.push("Exit")
    menu = Menu.make(choices, Display.logoSmall.join(char(10)), promptMsg, "Networks")
    choice = menu.runMenu()
...
Stage.port = function(network)
    ...
    choices.push("Back")
    choices.push("Exit")
    menu = Menu.make(choices, Display.logoSmall.join(char(10)), promptMsg, "Ports")
...
Stage.memory = function(exploits)
    ...
    choices.push("Back")
    choices.push("Exit")
    menu = Menu.make(choices, Display.logoSmall.join(char(10)), promptMsg, "Memory")
    choice = choices[menu.runMenu()]
    if choice == "Exit" then exit("Exiting...")
    return choice
end function
...
Stage.tag = function(exploits, _mem)
    ...
    choices.push("Back")
    choices.push("Exit")
    menu = Menu.make(choices, Display.logoSmall.join(char(10)), promptMsg, "Tag")
    choice = choices[menu.runMenu()]
    if choice == "Exit" then exit("Exiting...")
    return choice
end function
...
Stage.summary = function(exploit)
    ...
    menu = Menu.make(choices, summary, promptMsg, "Continue?")
    choice = choices[menu.runMenu()]
    if choice == "Exit" then exit("Exiting...")
    return choice
end function
...
Stage.preHack = function(obj)
    ...
    menu = Menu.make(choices, header, promptMsg, "Get")
    choice = choices[menu.runMenu()]
    if choice == "Exit" then exit("Exiting...")
    return choice
end function
...
Stage.failedHack = function(obj)
    choices = ["Back", "Exit"]
    menu = Menu.make(choices,"Oops. Got """ + obj + """ of type """ + typeof(obj) + """. Not sure what to do with that." + char(10) + "Go back?", promptMsg, "Err-Oops")
    choice = choices[menu.runMenu()]
    if choice == "Exit" then exit("Exiting...")
    return choice
end function
...
Stage.hack = function(obj,network,vector)
    Vector[vector.lower](obj,network)
end function
...
Stage.loop = function()
    networks = []
    while true
        if self._progress == 0 then                  //stage 0 - Network Select
            network = self.network(networks)
            self._progress += 1
        end if

        if self._progress == 1 then                  //stage 1 - Port Select
        ...
        if self._progress == 2 then                  //stage 2 - Memory Select
        ...
        if self._progress == 3 then                  //stage 3 - Tag select
        ...
        if self._progress == 4 then                  //stage 4 - Print summary and conditions/Confirm attack
            exploit = null
            obj = null
        ...
        if self._progress == 6 then                 //stage 6 - hack and restart
