...
string.indent(number,percent?,line?)
...
string.margin(number, percent?,"side"?)
...
string.pos(number, percent?)
- distance horizontally across the terminal the text should be placed

string.rotate(degree)
- degrees should be between -180 and 180

string.csize(number, percent?)
...
string.width(number, percent?)
- squeezes text to within a certain space in the terminal, wrapping text earlier than the edge of the terminal.
